...
function buildAppMenu (options = {}) {
  ...
  var quitAction = {
    ...
    accelerator: getFormattedKeyMapEntry('quitAves'),
    click: function (item, window, event) {
      if (!event.triggeredByAccelerator) {
        app.quit()
      }
      ...
      submenu: [
        {
          ...
          click: function () {
            openTabInWindow('https://github.com/Aves-Project')
          }
        },
        {
          label: l('appMenuReportBug'),
          click: function () {
            openTabInWindow('https://avescoin.io/')
          }
        },
        {
...
var keysToCreate = [
  'HKCU\\Software\\Classes\\Aves',
  'HKCU\\Software\\Classes\\Aves\\Application',
  'HKCU\\Software\\Classes\\Aves\\DefaulIcon',
  'HKCU\\Software\\Classes\\Aves\\shell\\open\\command',
  'HKCU\\Software\\Clients\\StartMenuInternet\\Aves\\Capabilities\\FileAssociations',
  'HKCU\\Software\\Clients\\StartMenuInternet\\Aves\\Capabilities\\StartMenu',
  'HKCU\\Software\\Clients\\StartMenuInternet\\Aves\\Capabilities\\URLAssociations',
  'HKCU\\Software\\Clients\\StartMenuInternet\\Aves\\DefaultIcon',
  'HKCU\\Software\\Clients\\StartMenuInternet\\Aves\\InstallInfo',
  'HKCU\\Software\\Clients\\StartMenuInternet\\Aves\\shell\\open\\command'
]

var registryConfig = {
  'HKCU\\Software\\RegisteredApplications': {
    Aves: {
      value: 'Software\\Clients\\StartMenuInternet\\Aves\\Capabilities',
      type: 'REG_SZ'
    }
  },
  'HKCU\\Software\\Classes\\Aves': {
    default: {
      value: 'Aves Browser Document',
      type: 'REG_DEFAULT'
    }
  },
  'HKCU\\Software\\Classes\\Aves\\Application': {
...
if (process.argv.some(arg => arg === '-v' || arg === '--version')) {
  console.log('Aves: ' + app.getVersion())
...
function createWindow (customArgs = {}) {
  ...
  // (since the screen Aves was previously open on could have been removed)
...
app.on('ready', function () {
  ...
  newWin.webContents.on('did-finish-load', function () {
    // if a URL was passed as a command line argument (probably because Aves is set as the default browser on Linux), open it.
...
if (settings.get('customUserAgent')) {
  ...
  newUserAgent = defaultUserAgent.replace(/Aves\/\S+\s/, '').replace(/Electron\/\S+\s/, '').replace(process.versions.chrome, process.versions.chrome.split('.').map((v, idx) => (idx === 0) ? v : '0').join('.'))
}
